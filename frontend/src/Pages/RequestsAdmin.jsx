...
const RequestsAdmin = () => {
      ...
      const aceptarPeticion = async () => {
        try {
          const response = await fetch(
            `http://localhost:8081/aceptarPeticion/${id}`,
            {
  ...
  function rechazarPeticion(id) {
    if (
      window.confirm("¿Estás seguro de que quieres rechazar esta petición?")
    ) {
      const aceptarPeticion = async () => {
        try {
          const response = await fetch(
            `http://localhost:8081/rechazarPeticion/${id}`,
            {
              method: "PUT", // Suponiendo que el método para aceptar la petición sea PUT
              headers: {
                "Content-Type": "application/json",
    ...
    <>
      ...
      <div className="resource-table">
        <div className="box-resource">
          ...
          <div className="search-resource"></div>
                    ...
                    <tr key={peticion.id_peticion} className="td-request-admin">
                      ...
                      <td>
                        <BsFillHandThumbsUpFill
                          ...
                          onClick={() => {
                            confirmarPeticion(peticion.id_peticion);
                          }}
                          color="green"
                          size={20}
                        />
                      ...
                      <td>
                        <BsFillHandThumbsDownFill
                          className="exit-request"
                          color="red"
                          size={20}
                          onClick={() => {
                            rechazarPeticion(peticion.id_peticion);
                          }}
                        />
                      </td>
                    </tr>
